...
Since v2.66.0, the admin can also pair up two already-registered solo entries directly, from Duplas — `mergeIntoTeam(catId, targetTeamId, sourceTeamId)`. This exists because neither of the two self-service paths could do it: the WhatsApp invite link only works when one side hasn't registered in that category yet, and a fresh "Duplas → Añadir" walk-in team leaves the two original solo rows orphaned instead of consuming them. It folds `sourceTeamId`'s single player into `targetTeamId` as `players[1]`, carrying over that player's *own* payment fields (`priceUsd`/`priceBs`/`paymentMethod`/`reference`/`proofName`/`paymentStatus`, renamed `joinedAt` from the source row's `createdAt`) — the exact shape `joinTeam` already produces for a self-service join — then deletes the source row; neither player's payment is touched. Same duplicate-id guard as `removePersonFromCategory`, and the same mixed-gender rule as `joinTeam` (never two of the same known gender in a `"mixto"` category).

`splitTeam(catId, teamId)` (v2.66.1) is the exact inverse — it un-merges a complete team, converting `players[1]`'s own payment fields back into a brand-new standalone team row (reversing the `joinedAt`/`createdAt` and field renames `mergeIntoTeam` did) and shrinking the original row back to just `players[0]`. It only works when `players[1].paymentStatus !== undefined` — a complete team the organizer typed in from scratch via `TeamRegistration`'s "Añadir" (both names at once, no checkout) has no per-player payment to split out, so `splitTeam` refuses rather than fabricate one. `TeamRegistration` shows "Desemparejar" next to any team where this applies, whether the pairing came from `mergeIntoTeam` or a real self-service join by link — both look identical once stored, and both are safe to split.
